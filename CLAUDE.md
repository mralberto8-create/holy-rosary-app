...
- **54-Day Novena** — button on home screen opens a full-screen info modal (history, both phase prayers, "How it works in this app") before starting; "I'm Ready — Begin Today" starts the novena; "Not Yet" closes without starting. Tracks start date in localStorage (`novena_54day`); novena follows the calendar (day = today minus start date). Shows status card on home screen (day, phase, progress bar, dates) while in progress. **No Level Up button** — the practice is to pray a fresh Rosary each day; if you miss a day, just return the next day and pray. Rosary progress (`rosary_progress`) is date-stamped and automatically discarded at midnight; each day starts fresh. During the rosary, when novena is active, a prayer popup appears after each Fatima Prayer (end of each decade) showing the Petition prayer (Days 1–27) or Thanksgiving prayer (Days 28–54); user taps Continue to advance. Works in both manual and Sleep Mode.
